...
6. **Re-read your own criteria before you return them.** Take each one and ask
   what single measurement fails it. A criterion joined by "and", "sowie" or a
   semicolon, or one listing three or more comma-separated requirements, is
   already two criteria — split it here rather than leaving it for the person
   who has to review it. This step is not optional politeness: the last run
   asked for one assertion each in exactly the words above and returned four
   compound criteria out of ten anyway.
