...
/**
 * Write via a temp file and rename, so a concurrent reader never sees a torn
 * file. A failed rename takes its staging file with it: the caller reports the
 * error, and this directory is meant to stay bounded either way.
 */
...
/** No counter moved: every number zero and no tool called. */
 ...
 *
 * All-zero counters are a statement — the model never touched the tools — so
 * they are never allowed to replace counters already frozen here. A session
 * idle past `ABANDONED_AFTER_MS` but still alive is swept by a newer one,
 * which stores its counters and deletes the journal they came from; when it
 * later ends properly there is nothing left to derive, and writing that would
 * turn a lost measurement into a false one (BCC-5).
 *
...
export function writeSessionSummary(repoRoot, sessionId, now = Date.now()) {
  ...
  const target = summaryPath(repoRoot, sessionId);
  if (emptyMetrics(metrics)) {
    try {
      const stored = JSON.parse(readFileSync(target, "utf8"));
      if (stored?.metrics) return stored.metrics;
    } catch {
      // Nothing readable to protect: an empty summary is a valid result and is
      // written below.
    }
  }
  mkdirSync(cacheDir(repoRoot), { recursive: true, mode: 0o700 });
  writeAtomic(target, JSON.stringify({ endedAt: now, metrics }));
 ...
 *   { t: "recorded" }                         — the edited paths reached `--modified-file`
...
export function deriveSession(repoRoot, sessionId) {
  ...
  events.forEach((event, i) => {
    if (event.t === "edit") {
      ...
      lastNotesIdx = i;
    } else if (event.t === "recorded") {
      // Recorded is what empties the pending list, and only the write that
      // puts the paths in `--modified-file` records them. The reset used to
      // hang off `notes` instead, so that the first task finished in a session
      // did not leave its files for the second to inherit (BCC-9) — but any
      // prose `--append-notes` writes that event too, and six of them mid-task
      // emptied the list before the finish that needed it. EDG-4 went Done
      // naming a file from an unrelated sweep and none of the thirteen edits
      // this session made (BCC-12). A path edited again afterwards is pending
      // again, which is why the seen-set is cleared too.
      pendingSeen.clear();
      ...
      else if (event.name === "taskless-continue") metrics.tasklessContinues += 1;
    }
  });
  ...
  // The cast, not the annotation, is what callers see: `taskId` is only ever
  // assigned inside the fold's callback, so control-flow analysis still reads
  // its declared type as the initialiser's `null` at this return.
  return {
    sourceEdits,
    pendingModifiedFiles: pendingOrder,
    editsAtLastNotes,
    stale,
    taskId: /** @type {string | null} */ (taskId),
    metrics,
  };
}
