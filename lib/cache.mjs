...
export function deriveSession(repoRoot, sessionId) {
  ...
  events.forEach((event, i) => {
    if (event.t === "edit") {
      ...
      lastNotesIdx = i;
      // Written down is written down: notes say what changed, so the edits
      // before them stop being pending. The turn_end guard's comment has said
      // this reset happens since the journal replaced the snapshot; the fold
      // never did it, and the first task finished in a session left its files
      // for the second one to inherit (BCC-9). A path edited again after the
      // notes is pending again, which is why the seen-set is cleared too.
      pendingSeen.clear();
