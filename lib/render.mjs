...
/**
 * Ready work, ranked by `rankReady`: our recommendation in a `notice()`, their
 * titles in a `dataBlock`. `total` is the size of the whole backlog, absent
 * when the caller did not count it.
 *
 * @param {import("./types.mjs").Task[]} tasks
 * @param {{ status?: string, total?: number }} [options]
 */
export function renderNext(tasks, { status = "To Do", total } = {}) {
  if (tasks.length === 0) {
    return notice(
      total === 0
        ? "This backlog holds no tasks at all, so nothing is blocked — the first one has yet to be written. " +
            "Create it with `backlog_task_create`, or without the tools:\n" +
            "  backlog task create '<title>' --ac '<first criterion>'\n\n" +
            "A task with no predecessor names no dependency; every task in an empty backlog is one."
