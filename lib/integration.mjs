...
import { initialisesBacklog, mutatesBacklog, writesTaskNotes } from "./bash.mjs";
...
/**
 * Refuse a `backlog init` that would land on a configuration that already
 * exists. Nothing else about a shell command is inspected here.
 *
 ...
 */
function initGuard(toolInput, cwd, guard, sessionId) {
  const command = toolInput && typeof toolInput === "object" ? Reflect.get(toolInput, "command") : undefined;
  if (!initialisesBacklog(command)) return null;
...
export function evaluateToolGuard({
  ...
  if (call.toolName === "bash") return initGuard(call.toolInput, cwd, guard, sessionId);
  ...
  for (const target of toolTargetPaths(call.toolInput)) {
    ...
    if (classification.kind === "decision" && existsSync(resolve(project.root, target))) continue;
    const decision = {
      block: guard !== "0",
